...
//函数参数类型声明,跟ZEND_FE配合使用
//第一个参数是结构体变量名，第二个参数是如果被赋值为1，则代表着所有的参数默认都是需要以引用的方式传递的(在arginfo中单独声明的除外)
//第三个参数如果为1,声明这个函数的返回值需要以引用的形式返回,第四个参数函数被调用时，传递参数至少为前N个函数，-1表示全部
...
PHP_METHOD(yaf_application, __construct) {
	...
	self = getThis();
        ...
        //开始解析参数,|：它之前的参数都是必须的，之后的都是非必须的，也就是有默认值的。
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "z|z", &config, &section) == FAILURE) {
       ...
       //输出string配置ini文件路径
        //php_printf("%s",Z_STRVAL_P(config));
...
*/
//实现yaf_application.h中声明的方法，最终解析为zm_startup_yaf_application(type, module_number TSRMLS_CC)
YAF_STARTUP_FUNCTION(application) {
	zend_class_entry ce; //声明一个类变量

       //YAF_INIT_CLASS_ENTRY对应的宏为  INIT_CLASS_ENTRY
       //YAF_INIT_CLASS_ENTRY中多了一个参数，用来区分命名空间类名称
       //INIT_CLASS_ENTRY 第一个参数是类变量，第二个参数是类名称，第三个参数是方法结构体变量
	YAF_INIT_CLASS_ENTRY(ce, "Yaf_Application", "Yaf\\Application", yaf_application_methods);

       //zend_register_internal_class注册类
       //zend_register_internal_class_ex定义子类

	yaf_application_ce = zend_register_internal_class_ex(&ce, NULL, NULL TSRMLS_CC);//继承null类，表示该类无父类
	yaf_application_ce->ce_flags |= ZEND_ACC_FINAL_CLASS; //声明该类为最终类

       //设置类的属性，第一个参数是类，第二个参数是属性名,第三个参数是属性名长度，第四个参数是属性值，第五个参数是访问级别属性
       //zend_declare_property_null只需要四个参数
       //函数原型为
       //ZEND_API int zend_declare_property_string(zend_class_entry *ce, char *name, int name_length, char *value, int access_type TSRMLS_DC);
       //ZEND_STRL是一个宏，返回属性名，和属性长度
