...
PHP_INI_BEGIN()
	...
	STD_PHP_INI_ENTRY("yaf.library",         	"aa",  PHP_INI_ALL, OnUpdateString, global_library, zend_yaf_globals, yaf_globals)
...
PHP_MINIT_FUNCTION(yaf)
{
	...
	REGISTER_INI_ENTRIES();
        ...
        //YAF_G()访问的是yaf_globals结构体的属性 
       // php_printf("%s",YAF_G(global_library));
 ...
 //解析 zm_startup_yaf_application(type, module_number TSRMLS_CC)   该函数在yaf_application.h中YAF_STARTUP_FUNCTION(application)中声明，在
 //yaf_application.c中实现 YAF_STARTUP_FUNCTION(application)中实现
